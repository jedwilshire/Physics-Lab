...
class Application:
    ...
    def detect_collisions(self):
        ...
        particles = self.particles_group.sprites()
        vel_multiplier = 10
        for i in range(len(particles)):
            partA = particles[i]
            for k in range(i + 1, len(particles)):
                partB = particles[k]
                vect = partA.pos - partB.pos
                if vect.length_squared() <= (partA.radius + partB.radius) ** 2:
                    partA.vel = partA.vel.reflect(vect)
                    partB.vel = partB.vel.reflect(vect)
                    partA.pos = partB.pos + vect * 1.1
                    partB.pos = partA.pos - vect * 1.1
