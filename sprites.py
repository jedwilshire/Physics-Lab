...
class Particle(pygame.sprite.Sprite):
    def __init__(self, application, x, y, speed, angle, radius, color):
        ...
        # create self.bouncing a bool set to True to indicate bouncing
        self.bouncing = True
    ...
    def update(self):
        ...
        if not self.touching_bottom_wall():
            ...
            self.acc.y = -GRAVITY
        if self.touching_bottom_wall() and self.vel.y < EFFECTIVE_ZERO:
            self.vel.y = 0
            self.acc.y = 0
        # if not self.bouncing, set vel.y and acc.y to 0
